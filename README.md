...
1. **모든 회차는 `PROTOCOL.md`의 공정성 규칙을 따른다.** (세션 격리, 입력 제한, 표준 응답 등)
2. 한 하니스 1회차마다 `runs/<harness>/notes.md`를 채운다 (모델 버전·날짜·사람 개입 횟수 등).
3. 산출물 코드/계획문서는 같은 폴더 안에 둔다.
4. 끝나면 `matrix.md`에 한 줄 추가 — 채점 시점에만 `spec/notes.md` 정답지를 연다.
5. 같은 하니스를 다시 돌릴 때는 `runs/<harness>/v2/`처럼 하위 폴더로 누적.
